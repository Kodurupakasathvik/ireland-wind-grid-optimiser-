...
Secondary criteria:
    Mean cross-window NMAE standard deviation
    Worst-case NMAE

IMPORTANT:
    A model can participate in model selection only when rolling-validation
    results are available for the required horizons.

Fixed benchmark results are retained for reference only.
"""
...
FEATURE_ENGINEERED_ROLLING_FILE = (
    PROCESSED
...
def canonical_horizon(value: Any) -> str | None:
    """
    Convert horizon representations into project-standard labels.
    """
...
def load_benchmark() -> pd.DataFrame:
    """
    Load fixed benchmark results.

    These results are informational only and cannot override
...
def load_rolling_summary(
    ...
    """
    Load rolling-validation summary.
    ...
    # -------------------------------------------------------------------------
    # Remove duplicate horizons
    #
    # A selector must have exactly one summary row per horizon.
    # -------------------------------------------------------------------------
...
def select_best_models(
    ...
    for horizon in HORIZONS:
        ...
        for model in ROLLING_MODELS:
            ...
            std_nmae = (
                float(row["std_nmae_percent"])
                if pd.notna(row["std_nmae_percent"])
                else float("inf")
            )

            worst_nmae = (
                float(row["worst_nmae_percent"])
                if pd.notna(row["worst_nmae_percent"])
                else float("inf")
            )
...
def build_overall_ranking(
    ...
    for model in ROLLING_MODELS:
        ...
        # Ignore NaN values when calculating stability.
        valid_std = valid[
            "std_nmae_percent"
        ].dropna()
        ...
        # Ignore NaN values for worst case.
        valid_worst = valid[
            "worst_nmae_percent"
        ].dropna()
    ...
    # -------------------------------------------------------------------------
    # Only fully validated models are eligible for the final recommendation.
    # -------------------------------------------------------------------------

    dataframe = dataframe[
        dataframe["horizons_evaluated"] == len(HORIZONS)
    ].copy()
...
def main() -> None:
    ...
    feature_engineered = load_rolling_summary(
        FEATURE_ENGINEERED_ROLLING_FILE,
        "Feature-Engineered LR",
    )
    ...
    rolling_results: dict[
        ...
        "Feature-Engineered LR": feature_engineered,
    ...
    if overall_ranking.empty:
        ...
        print(
            "No fully rolling-validated models are available."
        )
    ...
    # -------------------------------------------------------------------------
    # Models still missing rolling validation
    # -------------------------------------------------------------------------
    ...
    for model in MODELS:
        ...
        elif rolling["horizon"].nunique() < len(HORIZONS):
    ...
    if benchmark_only:
        ...
        print(
            "The following models do not yet have complete "
            "rolling-validation results:"
        )
